...
export async function movieRecommendation(movie: any, query: string) {
	...
	const movieJsonSchema = z.toJSONSchema(movieSchema)
	...
	if (!resp.output_text) throw new Error('No output text from the model.');

	const respMovie = movieSchema.parse(JSON.parse(resp.output_text));
